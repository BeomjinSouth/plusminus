...
- 부호 정리 단계는 각 항마다 `+ / -` 선택 버튼과 숫자 입력 칸을 따로 두고, 숫자 칸에 `-3`, `3`, `+3`처럼 입력한 부호가 버튼 선택과 자동 동기화되게 한다.
- 부호 정리 단계는 각 항 카드 아래에 현재 읽힌 항을 바로 보여 주고, 최종 식/이동 단계 리본은 `-3+8-4`처럼 정리된 부호 형태로 표시하며, 최종 식 입력은 `Enter` 제출도 지원한다.
- 부호 정리 단계의 항 카드와 최종 식 입력 영역은 가운데 정렬을 기본으로 한다.
...
- 수직선은 조작 대상이 분명해야 하므로 주변 장식보다 눈금과 현재 위치를 우선하고, 눈금 숫자는 크게 굵게 표시한다.
