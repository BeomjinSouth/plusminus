...
- 랜딩 화면은 1개의 핵심 메시지를 유지하되 플레이 루프, 보상감, 3개 모델의 짧은 미리보기를 함께 보여 준다
- 입장 화면은 설명 문단을 최소화하되, 입력 폼과 CTA 옆에 짧은 시작 순서 패널을 함께 배치한다
...
### 2026-03-22 UI 반영 메모
- 토끼 모델 `split-expression` 단계는 식을 문자 카드 단위로 벌리지 않고, 원식의 시각 형태를 유지한 채 아래 표시점으로만 끊기를 선택한다.
- 각 표시점은 선택 여부만 보여 주고, 실제 분리 결과는 별도의 미리보기 칩으로 즉시 확인한다.
- 모바일에서도 식 본문과 표시점이 한 묶음으로 유지되도록 가로 스크롤 가능한 패널로 렌더링한다.

### 2026-03-23 UI 반영 메모
- 공통 프레임은 상단에 `1인 미션 플레이` 배지와 짧은 제품 설명을 두어 대시보드보다 미션 앱에 가까운 인상을 준다.
- 랜딩은 `수학을 미션처럼 바로 시작하는 화면` 카피 아래 CTA, 플레이 루프, 모델별 미리보기 카드를 함께 배치한다.
- 입장 화면은 좌측 미션 안내 패널, 우측 입력 패널의 2단 구성을 사용한다.
