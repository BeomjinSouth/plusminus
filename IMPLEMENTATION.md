...
- `-` 가 홀수 개면 음수
- `-` 가 짝수 개면 양수
- 학생은 항별 결과와 함께 전체 `최종 식`도 직접 입력한다.

### 10-3. 이동 단계
- 시작점은 항상 `최종 식`의 첫 번째 항 위치
- 첫 번째 항에서 출발한 뒤, 남은 항의 절댓값만큼 이동
- 부호에 따라 방향 결정
- 현재 위치 검증 후 다음 단계 진입
...
- 토끼 `split-expression` 보드는 큰 블록, 가위 아이콘, 붉은 `/` 배지, 초록색 다시 붙이기 버튼으로 놀이형 조작감을 강하게 준다.
- 성공 피드백 배너는 메시지가 바뀔 때마다 짧은 반짝임 애니메이션을 다시 재생한다.
- 식 가독성을 위해 토끼 블록은 무채색 위주로 단순화하고, 가위 아이콘은 gap hover/focus 때만 드러낸다.

### 2026-03-23 토끼 흐름 조정 메모
- `normalize-signs` 단계는 항별 부호 정리 입력과 전체 `최종 식` 입력을 함께 검증한다.
- `choose-start-point` 단계는 `0`이 아니라 첫 번째 항 위치를 시작점으로 검증한다.
- `move` 단계는 긴 설명 카드 대신 `최종 식`에서 현재 적용할 항만 강조하고, 숫자선 마커는 토끼 애니메이션으로 렌더링한다.
...
### 2026-03-23 토끼 화면 단순화 메모
- 토끼 모델도 상단에는 `문제 번호`, `식`, `해야 할 일`만 크게 보여 준다.
- 토끼 본문은 단계별 설명문을 길게 두지 않고 `끊기`, `부호`, `시작`, `점프`, `답` 카드만 남긴다.
- 토끼 이동 단계에서는 현재 항, 방향, 칸 수를 카드로 먼저 보여 주고 그 아래에 수직선을 둔다.

### 2026-03-23 진행 잠금 구현 메모
- 학생별 진행 상태는 클라이언트 로컬 스토리지 `plusminus:student-progress` 에 `studentKey -> completedSetIds[]` 형태로 저장한다.
- `counting-stones-high` 세트 완료를 `돌 놓기 마스터` 조건으로 사용하고, 이 조건이 만족되면 `rabbit-sign-parser`(`점프 계산`) 진입을 허용한다.
- `postman`(`점수 카드`)는 `getModelAvailability` 규칙에서 항상 `disabled`를 반환하도록 두고, 로비와 `/play/postman/*` 직접 진입을 함께 막는다.
- 로비, 플레이 진입 페이지, 결과 페이지가 모두 같은 진행 규칙 유틸을 사용해 해금 상태를 판단한다.
